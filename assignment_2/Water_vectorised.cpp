...
void UpdateNonBondedForces(System& sys){
    ...
    for (long unsigned int j = i+1; j < molecule.no_mol; j++)
...
System MakeWater(int N_molecules){
    ...
    Molecules& molecule = sys.molecules;
    molecule.atoms.push_back(Oatom);
    molecule.atoms.push_back(Hatom1);
    molecule.atoms.push_back(Hatom2);
    molecule.bonds = waterbonds;
    molecule.angles = waterangle;
    molecule.no_mol = N_molecules;
